...
int numberOfIntersectionTests=0;
Vec3f* RayTracer::render(Camera* camera, Scene* scene, int nbounces, int sampling){
	...
	std::vector<Ray> rays;
	BVH* bvh = new BVH(scene->getShapes(), 8);
	bvh->buildTree();
	...
	for (int i = 0; i < camera->getWidth(); i++) {
		for (int j = 0; j < camera->getHeight(); j++) {
			...
			int k = bvh->intersectedObject;
			if (fabs(h.point0.x - 1e10) > 1e-4 && fabs(h.point0.y - 1e10) > 1e-4 && fabs(h.point0.z - 1e10) > 1e-4) {
				...
				for (int b = 0; b < lights.size(); b++) {
					...
					if (lights.at(b)->getType() == "pointlight") {
						...
						skip.push_back(k);
						bvh->dont_try = skip;
						Hit blockedH = bvh->intersect(lightRay);
						numberOfIntersectionTests++;
						if (fabs(blockedH.point0.x - 1e10) > 1e-4 && fabs(blockedH.point0.y - 1e10) > 1e-4 && fabs(blockedH.point0.z - 1e10) > 1e-4) {
						...
						for (int ii = 0; ii < cell; ii++) {
							for (int jj = 0; jj < cell; jj++) {
								...
								while (nsampling < sampling) {
									...
									skip.push_back(k);
									bvh->dont_try = skip;
									Hit blockedH = bvh->intersect(areaRay);
									numberOfIntersectionTests++;
									if (fabs(blockedH.point0.x - 1e10) > 1e-4 && fabs(blockedH.point0.y - 1e10) > 1e-4 && fabs(blockedH.point0.z - 1e10) > 1e-4) {
			...
			if (fabs(KR) > 1e-6) {
				...
				for (int b = 0; b < lights.size(); b++) {
					...
					while (n <= nbounces) {
						...
						Vec3f diffuse(0,0,0);
						skip.clear();
						skip.push_back(reflectedObject);
						bvh->dont_try = skip;
						Hit h = bvh->intersect(reflectedRay);
						int k = bvh->intersectedObject;
						numberOfIntersectionTests++;
						if (fabs(h.point0.x - 1e10) > 1e-4 && fabs(h.point0.y - 1e10) > 1e-4 && fabs(h.point0.z - 1e10) > 1e-4) {
							numberOfTrueIntersection++;
							...
							skip.push_back(k);
							bvh->dont_try = skip;
							Hit hh = bvh->intersect(rrRay);
							numberOfIntersectionTests++;
							if ((fabs(hh.point0.x - 1e10) > 1e-4 && fabs(hh.point0.y - 1e10) > 1e-4 && fabs(hh.point0.z - 1e10) > 1e-4) && dd > (hh.point0 - reflectedPoint).length()) {
								blocked = true;
								numberOfTrueIntersection++;
							}
							if (!blocked) {
								...
								if (lights.at(b)->getType() == "pointlight") {
									skip.clear();
									skip.push_back(k);
									skip.push_back(lastHitObject);
									bvh->dont_try = skip;
									Hit hshadow = bvh->intersect(shadowray);
									numberOfIntersectionTests++;
									if (fabs(hshadow.point0.x - 1e10) > 1e-4 && fabs(hshadow.point0.y - 1e10) > 1e-4 && fabs(hshadow.point0.z - 1e10) > 1e-4) {
										shadow = 0.0;
										numberOfTrueIntersection++;
									}
								}
	...
	std::cout << "numberOfTrueIntersection: " << numberOfTrueIntersection << ", " << "numberOfIntersectionTests: " << numberOfIntersectionTests << "\n";
	return pixelbuffer;
}
